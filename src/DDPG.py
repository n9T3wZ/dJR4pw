...
class OUNoiseParam:
    def __init__(self, model, theta=0.15, sigma=0.2, decay=0.999):
        self.theta = theta
        self.decay = decay
        self.sigma = sigma
        self.model = model
        self.state_dict = {name: torch.zeros_like(param) for name, param in model.named_parameters()}
        self.reset()

    def reset(self):
        for name in self.state_dict:
            self.state_dict[name].fill_(0)

    def noise(self):
        for name, param in self.model.named_parameters():
            dx = self.theta * -self.state_dict[name] + self.sigma * torch.randn_like(param)
            self.state_dict[name] += dx
...
class Actor(nn.Module):
    def __init__(self, num_cons, num_vars, N=16, K=2, num_cuts=1, hidden_channels=[32,32], use_shortcut=False, activation='StepSigmoid', squeeze='Sigmoid', use_bn=False, problem_type='knapsack'):
        """Initialize the ShortcutNet model"""
        ...
        if problem_type == 'knapsack':
            self.state_dim = N + K
            self.action_dim = num_cons*num_cuts + num_cuts*(num_cuts-1)//2
        elif problem_type == 'packing':
            self.state_dim = num_cons*num_vars + num_cons + num_vars
            self.action_dim = (num_cons+2*num_vars)*num_cuts + num_cuts*(num_cuts-1)//2
...
class DDPG:
    def __init__(
        ...
        if self.problem_type == 'knapsack':
            num_vars = N*K
            num_cons = N+K+2*num_vars 
        elif self.problem_type == 'packing':
            num_cons = N # m, number of constraints
            num_vars = K # n, number of variables
        
        self.actor = Actor(num_cons=num_cons, num_vars=num_vars, num_cuts=num_cuts, hidden_channels=hidden_channels_actor, activation=activation, squeeze=squeeze, use_bn = use_bn, problem_type=problem_type).to(device)
        ...
        self.target_actor = Actor(num_cons=num_cons, num_vars=num_vars, num_cuts=num_cuts, hidden_channels=hidden_channels_actor, activation=activation, squeeze=squeeze, use_bn = use_bn, problem_type=problem_type).to(device)
